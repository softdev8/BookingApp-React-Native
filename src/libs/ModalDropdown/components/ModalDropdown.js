...
export default class ModalDropdown extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      showDropdown: false,
      buttonText: props.defaultValue.Value,
      selectedIndex: props.defaultIndex
    };
  }

  componentWillReceiveProps(nextProps) {
    var buttonText = this._nextValue == null ? this.state.buttonText : this._nextValue.Value.toString();
    var selectedIndex = this._nextIndex == null ? this.state.selectedIndex : this._nextIndex;
    if (selectedIndex < 0) {
      selectedIndex = nextProps.defaultIndex;
      if (selectedIndex < 0) {
        buttonText = nextProps.defaultValue.Value;
      }
    }
    this._nextValue = null;
    this._nextIndex = null;
  ...
  select(idx) {
    ...
    console.log("Set button title--select-- <" + JSON.stringify(value.Value) + ">");
  }
  ...
  _onRowPress(rowData, sectionID, rowID, highlightRow) {
    if (!this.props.onSelect ||
      ...
      this.setState({
        buttonText: rowData.Value.toString(),
        selectedIndex: rowID
      });
      console.log("Set button title --onRowPress-- <" + JSON.stringify(rowData.Value) + ">");
    }
    if (!this.props.onDropdownWillHide ||
      this.props.onDropdownWillHide() !== false) {
      this.setState({
...
const styles = StyleSheet.create({
  ...
  dropdown: {
    position: 'absolute',
    width: AppSizes.screen.width - 110,
    marginLeft: 20,
